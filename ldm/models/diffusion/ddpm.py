...
class DDPM(pl.LightningModule):
    ...
    def __init__(self,
        ...
        assert parameterization in [
            "eps", "x0", "v"], 'currently only supporting "eps" and "x0" and "v"'
    ...
    def predict_start_from_z_and_v(self, x_t, t, v):
        ...
        return (
            ...
            extract_into_tensor(
                self.sqrt_one_minus_alphas_cumprod, t, x_t.shape) * v
        )
    ...
    def predict_eps_from_z_and_v(self, x_t, t, v):
        return (
            ...
            extract_into_tensor(
                self.sqrt_one_minus_alphas_cumprod, t, x_t.shape) * x_t
        )
    ...
    def get_v(self, x, noise, t):
        return (
            ...
            extract_into_tensor(
                self.sqrt_one_minus_alphas_cumprod, t, x.shape) * x
        )
...
class LatentDiffusion(DDPM):
    ...
    def __init__(self,
        ...
        self.cond_stage_key_text = cond_stage_key_text
        try:
            self.num_downs = len(
    ...
    def get_learned_conditioning(self, image):
        if self.cond_stage_forward is None:
            if hasattr(self.cond_stage_model, 'encode') and callable(self.cond_stage_model.encode):
                c = self.cond_stage_model.encode(image)
                if isinstance(c, DiagonalGaussianDistribution):
                    c = c.mode()
            else:
                c = self.cond_stage_model(image)
        else:
    ...
    def get_input(self, batch, k, return_first_stage_outputs=False, force_c_encode=False,
        ...
        if self.model.conditioning_key is not None and not self.force_null_conditioning:
            if cond_key is None:
                ...
                cond_key_text = self.cond_stage_key_text
    ...
    def forward(self, x, c, *args, **kwargs):
        ...
        if self.model.conditioning_key is not None:
            ...
            if self.cond_stage_trainable:
                c['c_crossattn'][0] = self.get_learned_conditioning(
                    c['c_crossattn'][0])
    ...
    def log_images(self, batch, N=8, n_row=4, sample=True, ddim_steps=50, ddim_eta=0., return_keys=None,
        ...
        if self.model.conditioning_key is not None:
            if hasattr(self.cond_stage_model, "decode"):
                ...
                try:
                    xc = log_txt_as_img(
                        (x.shape[2], x.shape[3]), batch["human_label"], size=x.shape[2] // 25)
                    log['conditioning'] = xc
                except KeyError:
...
class DiffusionWrapper(pl.LightningModule):
    def __init__(self, diff_model_config, conditioning_key):
        ...
        assert self.conditioning_key in [
            None, 'concat', 'crossattn', 'hybrid', 'adm', 'hybrid-adm', 'crossattn-adm']
...
class LatentInpaintDiffusion(LatentFinetuneDiffusion):
    ...
    def get_input(self, batch, k, cond_key=None, bs=None, return_first_stage_outputs=False):
        ...
        for ck in self.concat_keys:
            cc = rearrange(
                batch[ck], 'b h w c -> b c h w').to(memory_format=torch.contiguous_format).float()
            if bs is not None:
                cc = cc[:bs]
